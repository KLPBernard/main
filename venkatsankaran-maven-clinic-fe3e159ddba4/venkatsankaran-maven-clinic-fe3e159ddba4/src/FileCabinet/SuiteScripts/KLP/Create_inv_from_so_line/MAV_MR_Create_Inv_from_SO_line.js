   ...
   */
    (record, search, runtime, email, error) => {
        ...
        const getInputData = (inputContext) => {
            ...
            try {
                const postingPeriod = scriptObj.getParameter({
                    name: 'custscript_mr_inv_crt_frm_so_period'
                });
                const searchId = scriptObj.getParameter({
                    name: 'custscript_mr_inv_crt_frm_saved_search'
                });
                ...
                searchObj.filters.push(
                    search.createFilter({
                        name: 'custrecord_klp_isnd_posting_period',
                ...
                var recepient = scriptObj.getParameter({
                    name: 'custscript_mr_inv_crt_frm_so_recipient'
                });
                log.error({
                    title: 'ERROR',
        ...
        const map = (mapContext) => {
            ...
            const invFromSoRecId = mapVal.id
            const soId = mapVal.values.custrecord_klp_isnd_so_no.value;
            log.debug('$$SOID**', soId)
            const soText = mapVal.values.custrecord_klp_isnd_so_no.text;
            // log.debug('invSfId', invSfId);
            // const assertVal = mapVal.values.custrecord_klp_sfassetid
            var invSoRec = record.load({
                type: 'customrecord_klp_so_no_details',
                id: invFromSoRecId
            });
            invSoRec.save({
                enableSourcing: true,
            ...
            const invSearchRes = search.lookupFields({
                type: 'customrecord_klp_so_no_details',
                id: invFromSoRecId,
                columns: ['custrecord_klp_isnd_error_on_processing']
            });
            log.debug('After save val', invSearchRes.custrecord_klp_isnd_error_on_processing)
            if (invSearchRes.custrecord_klp_isnd_error_on_processing) {
                var err = error.create({
                    message: soText + '~' + invSearchRes.custrecord_klp_isnd_error_on_processing,
                    name: 'Validation Error'
                });
                record.submitFields({
                    type: 'customrecord_klp_so_no_details',
        ...
        const reduce = (reduceContext) => {
            ...
            log.debug('in Reduce', soId + lineValues)
            try {
                ...
                const form = scriptObj.getParameter({
                    name: 'custscript_mr_inv_crt_frm_so_form'
                });
                const headerValue = JSON.parse(lineValues[0]).values;
                // const invSfId = headerValue.internalid.value;
                var invSfArray = [];
                const period = headerValue.custrecord_klp_isnd_posting_period.value;
                var invDate = headerValue.custrecord_klp_isnd_inv_posting_period;
                if (invDate) {
                ...
                lineValues.forEach(element => {
                    ...
                    log.debug('elementVal', elementVal)
                    invCrtDetails[elementVal.custrecord_klp_isnd_so_line_no] = {
                        quantity: elementVal.custrecord_isnd_inv_qty,
                        postingPeriod: elementVal.custrecord_klp_isnd_inv_posting_period,
                        startDate: elementVal.custrecord_klp_isnd_svc_start_date,
                        endDate: elementVal.custrecord_klp_isnd_svc_end_date
                    }
                ...
                const lineArray = Object.keys(invCrtDetails);
                ...
                //         var emailArr = emailArr.join(";").toString();
                //         invRecord.setValue({
                //             fieldId: 'email',
                //             value: emailArr,
                ...
                for (var index = itemLineCount - 1; index >= 0; index--) {
                    ...
                    if (lineArray.includes(lineId)) {
                        log.debug('invCrtDetails[lineId]', invCrtDetails[lineId])
                        var startDate = invCrtDetails[lineId].startDate
                        var endDate = invCrtDetails[lineId].endDate
                        log.debug('Line ID' + lineId, invCrtDetails[lineId].quantity)
                        invRecord.setSublistValue({
                            sublistId: 'item',
                            fieldId: 'quantity',
                            line: index,
                            value: invCrtDetails[lineId].quantity
                        })
                        if (startDate)
                            invRecord.setSublistValue({
                                sublistId: 'item',
                ...
                if (invId) {
                    invSfArray.forEach(element => {
                        record.submitFields({
                            type: 'customrecord_klp_so_no_details',
                            id: element,
                            values: { 'custrecord_klp_isnd_created_inv_no': invId },
                            options: {
                                enableSourcing: true,
                                ignoreMandatoryFields: true
                            }
                ...
                const SOText = JSON.parse(lineValues[0]).values.custrecord_klp_isnd_so_no.text
                lineValues.forEach(element => {
                    const elementVal = JSON.parse(element).values;
                    // lineArr.push(elementVal.custrecord_klp_isnd_so_line_no)
                    crtInvRecArr.push(elementVal.internalid.value)
                });
                var errorObj = error.create({
                    message: crtInvRecArr.toString() + '~' + SOText + '~' + e.message,
                    name: 'Error Message'
                });
                throw errorObj;
            }
        ...
        const summarize = (summaryContext) => {
            ...
            var author = scriptObj.getParameter({
                name: 'custscript_mr_inv_crt_frm_so_author'
            });
            var recepient = scriptObj.getParameter({
                name: 'custscript_mr_inv_crt_frm_so_recipient'
            });
            ...
            summaryContext.reduceSummary.errors.iterator().each(function (key, error) {
                ...
                recToUpdate.forEach(element => {
                    record.submitFields({
                        ...
                        id: element,
                        values: {
                            custrecord_klp_isnd_error_on_processing: errormessage[2],
                            isinactive: true
                        },
                        options: {
                            enableSourcing: true,
